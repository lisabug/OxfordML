...
local counter = 0
local batch_inputs = torch.CudaTensor(opt.batch_size, train.data:size(2)):zero()
local batch_targets = torch.CudaTensor(opt.batch_size):zero()
local feval = function(x)
  ...
  batch_inputs:copy(train.data[{{start_index, end_index}, {}}])

  batch_targets:copy(train.labels[{{start_index, end_index}}])
  gradParameters:zero()
...
for i = 1, iterations do
  ...
  local testOut = model:forward(test.data)
  local testLoss = criterion:forward(testOut, test.labels)
  ...
  if i % 100 == 0 then -- don't print *every* iteration, this is enough to get the gist
      print(string.format("minibatches processed: %6s, trainLoss = %6.6f", i, minibatch_loss[1]))
      print(string.format("minibatches processed: %6s, testLoss = %6.6f", i, testLoss))
  end
...
gnuplot.plot(
  {'trainLoss',torch.range(1, #losses), torch.Tensor(losses),'-'},       -- x-coordinates for data to plot, creates a tensor holding {1,2,3,...,#losses}-- y-coordinates (the training losses)
   {'testLoss', torch.range(1, #testLosses), torch.Tensor(testLosses), '-'})
...
for i = 1, test.data:size(1) do
   if tonumber(classPredictions[i][1]) == tonumber(test.labels[i]) then
       predctions = predctions + 1
   end
end
print(predctions)
local classificationError = 1.0 - predctions / test.data:size(1)
print(string.format("classification error = %6.6f", classificationError))
